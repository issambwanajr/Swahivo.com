...
export function Dashboard({ user }: DashboardProps) {
  const [activeTab, setActiveTab] = useState<'chat' | 'overview' | 'notes' | 'profile' | 'analytics'>('chat');
  ...
  const sendMessage = async () => {
    ...
    setTimeout(() => {
      setMessages(prev => [...prev, {
        role: 'assistant',
        content: `I've received your ${selectedFile ? `file (${selectedFile.name}) and ` : ''}message: "${input}". How can I help you further?`
      }]);
    }, 1000);
  };
  ...
  return (
    <div className="flex h-screen bg-[#0d0d0f] text-text-main font-sans overflow-hidden">
      {/* Sidebar */}
      <aside className={cn(
        "border-r border-white/5 bg-[#0d0d0f] flex flex-col h-full transition-all duration-300 z-50",
      ...
      {/* Main Content */}
      <main className="flex-1 flex flex-col bg-[#0d0d0f] relative overflow-y-auto">
        {/* Top Header */}
        <header className="h-14 flex items-center justify-between px-6 sticky top-0 bg-[#0d0d0f]/80 backdrop-blur-md z-30">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3 text-text-dim">
              <button className="hover:text-text-main transition-colors"><UserPlus size={20} /></button>
        ...
        <div className="flex-1 flex flex-col items-center p-6 lg:p-12">
          {activeTab === 'chat' && (
            <div className="w-full max-w-3xl flex-1 flex flex-col items-center justify-center gap-8">
              {messages.length === 0 ? (
                ...
                <div className="w-full max-w-2xl relative group">
                  <div className="absolute inset-0 bg-accent/5 blur-2xl rounded-full opacity-0 group-focus-within:opacity-100 transition-opacity" />
                    ...
                    {isPlusMenuOpen && (
                      <>
                        ...
                        <motion.div
                          ...
                          className="absolute bottom-full left-0 mb-4 z-50 bg-[#171717] border border-white/10 rounded-2xl shadow-2xl p-2 min-w-[200px]"
                        >
                          <button 
                  ...
                  <div className="relative flex items-center bg-[#171717] border border-white/5 rounded-2xl py-4 pl-12 pr-24 text-white placeholder:text-text-dim outline-none focus:border-white/20 transition-all shadow-2xl">
                ...
                <div className="bg-linear-to-br from-accent to-accent-light p-8 rounded-[20px] text-white shadow-xl shadow-accent/20 flex flex-col justify-between min-h-[300px]">
                  <div>
                    ...
                    <p className="text-white/80 text-[0.95rem] leading-relaxed">
                      Unlock advanced analytics and unlimited NoteFlow storage.
                    </p>
                  </div>
          ...
          {/* Other tabs (notes, analytics, profile) */}
          {activeTab === 'notes' && (
            <div className="w-full max-w-5xl">
              <h2 className="text-3xl font-serif font-semibold mb-6">NoteFlow</h2>
